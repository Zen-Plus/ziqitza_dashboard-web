  ...
  return (
    <div className="EdsDashboardFilterWrap">
      ...
      <Dropdown
        ...
        placement={placement}
        overlayClassName={`${placement === 'bottomRight' ? 'EdsDashboardListFilter-Position-right' : 'EdsDashboardListFilter-Position-left'} ${wrapperClassName}`}
      >
        <Button
          style={{ background: 'transparent', width: '40px', padding: '10px' }}
          onClick={() => {
            if (isScrollIntoView) {
              const element = document.querySelector('.EdsDashboardFilterWrap');
